...
def save_data(queue: queue.Queue):
    ...
    tts=[]
    while True:
        [x,y,volt,temp,t]=queue.get()
        queue.task_done()
        try:
            l = len(xs)     
            if (l >= block_length):#or gap:
                streamBuilder_tmp(network, station, location, 'LAY', xs, len(xs), tts[0],
                            1 , 1)
                streamBuilder_tmp(network, station, location, 'LAX', ys, len(ys), tts[0],
                                1, 1)
                streamBuilder_tmp(network, station, location, 'LQV', volts, len(volts), tts[0],
                                1, 1)
                streamBuilder_tmp(network, station, location, 'LKD', temps, len(temps), tts[0],
                                1, 1)
                tts=[]
                xs=[]
                ys=[]
...
#HOST = "unifigdc.dyndns.org"  # The server's hostname or IP address
#PORT = 954  # The port used by the server
...
while True:
    try:
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            while True:
                ss=s.recv(100)
                datastr+=''.join([chr(d) for d in ss])
                w=datastr.split('\r\n')
                for ww in w:
                    if len(ww)==42:
                        x,y,volt,temp,ttlt=decrypt(ww)
                        if tsync:
                            tx=UTCDateTime.now()
                            tx.microsecond=0
                            t0=ttlt
                            tsync=False
                        t=tx+ttlt-t0
                        # if np.abs(t-UTCDateTime.now())>200:
                        #     tsync=True
                        print(t,'   ',ttlt,'   ',UTCDateTime.now())
                        queueLst.put([x,y,volt,temp,t])
                    else:
                        datastr=ww
    except:
        print('e')
